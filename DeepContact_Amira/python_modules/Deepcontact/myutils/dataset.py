...
from myutils.utils import labelme_load_mask_one, labelme_load_mask


class MitochondrionDataset(utils.Dataset):

    def load_Mitochondrion(self, dataset_dir='/data/yangshuxin/data/mito_data', subset=''):
        ...
        name_list = glob(dataset_dir + '*.png', recursive=True) + \
                    ...
                    glob(dataset_dir + '*.jpeg', recursive=True)
    ...
    def load_mask(self, image_id, mask_name=None):
        ...
        if os.path.exists(json_data):
            if mask_name == None:
                ...
                mask = labelme_load_mask(json_data, [mask_name], img_shape)
            else:
                mask = labelme_load_mask_one(json_data, [mask_name], img_shape)
        else:
            mask = np.ones(img_shape)
